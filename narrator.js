/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Min Huna Na'lam
...
(function() {
  ...
  function extractCards(panel) {
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.knowledge-card').forEach(el => {
        if (el.style.display === 'none') return;
        const t = el.querySelector('.knowledge-title');
        const d = el.querySelector('.knowledge-desc');
        const m = el.querySelector('.misc-text');
        const r = el.querySelector('.resp-text');
        const v = el.querySelector('.verse-arabic');
        let text = '';
        if (t) text += t.textContent + '. ';
        if (d) text += d.textContent + '. ';
        if (m) text += m.textContent + '. ';
        if (r) text += r.textContent + '. ';
        if (v) text += v.textContent;
        cards.push({ el, text: cleanText(text), type: 'knowledge' });
      });
    } else if (tabName === 'path') {
      panel.querySelectorAll('.path-stage').forEach(el => {
        const text = cleanText(el.textContent);
        cards.push({ el, text, type: 'path' });
      });
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Min Huna Na'lam/gi, 'Mine Houna Nalam')
        .replace(/Khalid Muhammad Khalid/gi, 'Khalid Mohamèd Khalid')
        .replace(/Montesquieu/gi, 'Monteskieu')
        .replace(/Al-Azhar/gi, 'al Azar').replace(/Faisal/gi, 'Faycal')
        .replace(/\bet\b/g, ', et').replace(/\bmais\b/g, ', mais')
        .replace(/\bou\b/g, ', ou').replace(/\bcar\b/g, ', car')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Min Huna Na'lam/gi, 'Min Hoona Nah-lam')
        .replace(/Khalid Muhammad Khalid/gi, 'Khah-lid Muhammad Khah-lid')
    ...
    if (l === 'ar') {
      clean = clean
        .replace(/Montesquieu/gi, 'مونتسكيو')
        ...
        .replace(/\bأو\b/g, '، أو').replace(/\bثم\b/g, '، ثم')
        .replace(/\bبل\b/g, '، بل');
    }
  ...
  function readCurrentCard() {
    ...
    if (card.type === 'knowledge' && card.el && !card.el.classList.contains('open')) { card.el.classList.add('open'); }
    updateProgress();
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    const knowledgeId = card.el ? card.el.id : '';
    const kMatch = knowledgeId.match(/knowledge-(\d+)/);
    if (kMatch && typeof KNOWLEDGE !== 'undefined') {
      const k = KNOWLEDGE.find(item => item.id === parseInt(kMatch[1]));
      if (k && k[duoLang]) { duoText = k[duoLang].title + '. ' + k[duoLang].desc; }
    }
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'من هنا نعلم' : l === 'fr' ? 'C\'est Ici Que Nous Apprenons' : 'This Is Where We Learn',
  ...
  function updateUI() {
    const btn = document.getElementById('narratorMainBtn'); const playBtn = document.getElementById('narratorPlayPause');
  ...
  // ═══ SETTINGS HANDLERS ═══
  function onSpeedChange(val) { STATE.speed = parseFloat(val); const label = document.getElementById('narratorSpeedLabel'); if (label) label.textContent = STATE.speed + 'x'; localStorage.setItem('mhn-narrator-speed', STATE.speed); }
  function onPitchChange(val) { STATE.pitch = parseFloat(val); const label = document.getElementById('narratorPitchLabel'); if (label) label.textContent = STATE.pitch.toFixed(1); localStorage.setItem('mhn-narrator-pitch', STATE.pitch); }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('mhn-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('mhn-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('mhn-narrator-duo', checked); }

  function populateVoiceSelect() {
    const select = document.getElementById('narratorVoice'); if (!select) return;
    const voices = speechSynthesis.getVoices(); const l = getLang();
    ...
    filtered.forEach(function(item) {
      const opt = document.createElement('option'); opt.value = item.idx;
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('mhn-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('mhn-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('mhn-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('mhn-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('mhn-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = ['.about-disclaimer', '.about-author', '.about-section', '.knowledge-card', '.path-stage', '.daily-card'];
  ...
  var origRenderKnowledge = window.renderKnowledge;
  if (origRenderKnowledge) { window.renderKnowledge = function() { origRenderKnowledge(); setTimeout(injectSpeakButtons, 100); }; }
  var origRenderPath = window.renderPath;
  if (origRenderPath) { window.renderPath = function() { origRenderPath(); setTimeout(injectSpeakButtons, 100); }; }
  ...
  function initNarrator() {
    loadSettings(); updateLabels(); setTimeout(injectSpeakButtons, 500);
